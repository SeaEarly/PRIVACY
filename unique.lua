...
-- Function to create player selection UI
local function showPlayerSelection()
    local playerListGui = Instance.new("ScreenGui", LocalPlayer:WaitForChild("PlayerGui"))
    local frame = Instance.new("Frame", playerListGui)
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            button.MouseButton1Click:Connect(function()
                ...
                DiscordLib:Notification("Notification", "Activated Aim Viewer on " .. player.Name, "Okay!")
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.End and UserInputService:IsKeyDown(Enum.KeyCode.LeftControl) then
        stopAimViewer()
